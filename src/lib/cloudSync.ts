...
export const cloudSync = {
  ...
  async persistCaseUpdate(u: CaseUpdateRecord) {
    ...
    const isUuid = /^[0-9a-f-]{36}$/i.test(u.caseId);
    const query = supabase.from("persons").select("id");
    const res = await safe(
      (isUuid ? query.eq("id", u.caseId) : query.eq("public_case_code", u.caseId)).maybeSingle(),
    );
